  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      if (Array.isArray(response)) {
        ...
        } else if ('data' in response && response.data && typeof response.data === 'object' && 'items' in response.data) {
          notifData = response.data.items as any[]
        }
      }
      ...
      const isSuccess = typeof response === 'object' && response && 'success' in response 
        ? (response as ApiResponse<any>).success 
        : Array.isArray(response) || (response && typeof response === 'object');
        
      if (isSuccess) {
        setNotifications(notifData || [])
      } else {
        const errorMessage = typeof response === 'object' && response && 'message' in response 
          ? (response as ApiResponse<any>).message 
          : "获取通知失败";
