  ...
  const fetchSearchResults = async () => {
    ...
    try {
      ...
      if (response.success) {
        // 如果是"全部"标签，直接设置所有结果
        if (activeTab === 'all') {
          setResults(response.data || generateMockResults(searchQuery))
        } else {
          // 如果是特定标签，只更新该标签的结果
          const tabData = response.data || generateMockResults(searchQuery);
          if (activeTab in tabData) {
            setResults(prev => ({
              ...prev,
              [activeTab]: tabData[activeTab as keyof typeof tabData]
            }));
          }
        }
      } else {
