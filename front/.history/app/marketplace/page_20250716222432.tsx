...
export default function Marketplace() {
  ...
  const fetchFavoriteStatus = async () => {
    try {
      const response = await apiClient.getUserFavorites('market')
      if (response.success) {
        const favorites = new Set(response.data.map((item: any) => item.id))
  ...
  const handleFavorite = async (e: React.MouseEvent, itemId: string) => {
    ...
    try {
      ...
      // 调用API
      if (isFavorited) {
        await apiClient.unfavoriteItem(itemId, 'market')
        toast.success("已取消收藏")
      } else {
        await apiClient.favoriteItem(itemId, 'market')
        toast.success("收藏成功")
      }
    } catch (error) {
            ...
            <Link href={`/marketplace/${item.id}`} key={item.id}>
              ...
              <div className="relative">
                ...
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className={`absolute top-2 right-2 bg-white/80 hover:bg-white ${favoritedItems.has(item.id) ? 'text-red-500' : ''}`}
                  onClick={(e) => handleFavorite(e, item.id)}
                >
                  <Heart className={`h-4 w-4 ${favoritedItems.has(item.id) ? 'fill-red-500' : ''}`} />
                </Button>
