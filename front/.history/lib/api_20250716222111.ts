...
class ApiClient {
  ...
  // 通用点赞功能
  async likePost(id: string, type: 'post' | 'confession' | 'market' | 'event' = 'post'): Promise<ApiResponse<null>> {
    return this.request<ApiResponse<null>>(`/${type}s/${id}/like`, {
      method: 'POST',
    });
  }
  
  // 取消点赞
  async unlikePost(id: string, type: 'post' | 'confession' | 'market' | 'event' = 'post'): Promise<ApiResponse<null>> {
    return this.request<ApiResponse<null>>(`/${type}s/${id}/unlike`, {
      method: 'POST',
    });
  }
  
  // 检查是否已点赞
  async checkLikeStatus(id: string, type: 'post' | 'confession' | 'market' | 'event' = 'post'): Promise<ApiResponse<{liked: boolean}>> {
    return this.request<ApiResponse<{liked: boolean}>>(`/${type}s/${id}/like-status`);
  }

  // 收藏功能
  async favoriteItem(id: string, type: 'post' | 'market' | 'event' | 'course' = 'post'): Promise<ApiResponse<null>> {
    return this.request<ApiResponse<null>>(`/${type}s/${id}/favorite`, {
      method: 'POST',
    });
  }
  
  // 取消收藏
  async unfavoriteItem(id: string, type: 'post' | 'market' | 'event' | 'course' = 'post'): Promise<ApiResponse<null>> {
    return this.request<ApiResponse<null>>(`/${type}s/${id}/unfavorite`, {
      method: 'POST',
    });
  }
  
  // 检查是否已收藏
  async checkFavoriteStatus(id: string, type: 'post' | 'market' | 'event' | 'course' = 'post'): Promise<ApiResponse<{favorited: boolean}>> {
    return this.request<ApiResponse<{favorited: boolean}>>(`/${type}s/${id}/favorite-status`);
  }
  
  // 获取用户收藏列表
  async getUserFavorites(type?: 'post' | 'market' | 'event' | 'course'): Promise<ApiResponse<any[]>> {
    const endpoint = type ? `/user/favorites/${type}s` : '/user/favorites';
    return this.request<ApiResponse<any[]>>(endpoint);
  }

  // 搜索功能
  async search(params: SearchParams & { type?: string }): Promise<ApiResponse<any>> {
    const queryString = params ? `?${new URLSearchParams(params as any).toString()}` : '';
    return this.request<ApiResponse<any>>(`/search${queryString}`);
  }
  
  // 搜索用户
  async searchUsers(keyword: string, params?: PaginationParams): Promise<ApiResponse<User[]>> {
    const queryParams = { ...params, keyword };
    const queryString = `?${new URLSearchParams(queryParams as any).toString()}`;
    return this.request<ApiResponse<User[]>>(`/search/users${queryString}`);
  }
  
  // 搜索帖子
  async searchPosts(keyword: string, params?: PaginationParams): Promise<ApiResponse<any[]>> {
    const queryParams = { ...params, keyword };
    const queryString = `?${new URLSearchParams(queryParams as any).toString()}`;
    return this.request<ApiResponse<any[]>>(`/search/posts${queryString}`);
  }
  
  // 搜索课程
  async searchCourses(keyword: string, params?: PaginationParams): Promise<ApiResponse<Course[]>> {
    const queryParams = { ...params, keyword };
    const queryString = `?${new URLSearchParams(queryParams as any).toString()}`;
    return this.request<ApiResponse<Course[]>>(`/search/courses${queryString}`);
  }
  
  // 搜索活动
  async searchEvents(keyword: string, params?: PaginationParams): Promise<ApiResponse<Event[]>> {
    const queryParams = { ...params, keyword };
    const queryString = `?${new URLSearchParams(queryParams as any).toString()}`;
    return this.request<ApiResponse<Event[]>>(`/search/events${queryString}`);
  }
  
  // 搜索闲置物品
  async searchMarketItems(keyword: string, params?: PaginationParams): Promise<ApiResponse<MarketItem[]>> {
    const queryParams = { ...params, keyword };
    const queryString = `?${new URLSearchParams(queryParams as any).toString()}`;
    return this.request<ApiResponse<MarketItem[]>>(`/search/marketplace${queryString}`);
  }

  // 聊天相关API
  async createChatSession(userId: string): Promise<ApiResponse<{sessionId: string}>> {
    return this.request<ApiResponse<{sessionId: string}>>('/chats', {
      method: 'POST',
  ...
  async getChatSessions(): Promise<ApiResponse<any[]>> {
    return this.request<ApiResponse<any[]>>('/chats');
  }
  
  async getChatMessages(sessionId: string, params?: {page?: number, limit?: number}): Promise<ApiResponse<any[]>> {
    const queryString = params ? `?${new URLSearchParams(params as any).toString()}` : '';
    return this.request<ApiResponse<any[]>>(`/chats/${sessionId}/messages${queryString}`);
  }
  
  async sendChatMessage(sessionId: string, content: string): Promise<ApiResponse<any>> {
    return this.request<ApiResponse<any>>(`/chats/${sessionId}/messages`, {
      method: 'POST',
      body: JSON.stringify({ content }),
    });
  }
  
  async markChatAsRead(sessionId: string): Promise<ApiResponse<null>> {
    return this.request<ApiResponse<null>>(`/chats/${sessionId}/read`, {
      method: 'POST',
    });
  }
  
  async getUnreadChatCount(): Promise<ApiResponse<{count: number}>> {
    return this.request<ApiResponse<{count: number}>>('/chats/unread-count');
  }

  // 联系卖家 - 针对闲置市场
  async contactSeller(itemId: string): Promise<ApiResponse<{sessionId: string}>> {
    return this.request<ApiResponse<{sessionId: string}>>(`/marketplace/${itemId}/contact`, {
      method: 'POST',
    });
  }
